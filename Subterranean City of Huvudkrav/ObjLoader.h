...
private:
	...
	void readVertex();
	void readFace(vector<SimpleMeshVertex> *vertices);
	...
	void readSmooth();
	void generateNormals(vector<SimpleMeshVertex> *vertices);
public:
	...
	bool LoadMesh(vector<SimpleMeshVertex> *outVertices, vector<IndexGroup> *outIGroups,
				vector<Material> *outMaterials, const char *filename, vector<string> *texturePaths);
	void generateTangents(vector<MeshVertex> *vertices);
	void generateTangents2(vector<MeshVertex> * vertices);
	~ObjLoader();
};
